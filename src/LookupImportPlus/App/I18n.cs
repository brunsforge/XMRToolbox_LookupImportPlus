...
namespace LookupImportPlus.App
{
    ...
    public static class I18n
    {
        ...
        private static readonly Dictionary<string, Entry> Dict = new Dictionary<string, Entry>
        {
            ...
            ["ed.match2"] = new Entry("Business key — a unique alternate value (e.g. account number). Requires a suitable unique key attribute on the target.", "Business Key — ein eindeutiger Alternativwert (z. B. Kontonummer). Setzt ein geeignetes eindeutiges Schlüssel-Attribut am Ziel voraus."),
            ["ed.match3"] = new Entry("Search field — the visible Excel value plus optional conditions on the target.", "Suchfeld — der sichtbare Excel-Wert plus optionale Bedingungen auf dem Ziel."),
            ["ed.matchConflict"] = new Entry("No hit or several → escalate to the conflict list, skip, or fail the row (your choice below).", "Kein oder mehrere Treffer → an die Konfliktliste eskalieren, überspringen oder Zeile fehlschlagen (deine Wahl unten)."),
